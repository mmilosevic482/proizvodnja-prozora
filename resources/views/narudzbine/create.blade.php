            ...
            <form action="{{ route('narudzbine.store') }}" method="POST">
                        ...
                        <select class="form-control @error('klijent_id') is-invalid @enderror"
                            ...
                            @foreach($klijenti as $klijent)
                                <option value="{{ $klijent->id }}"
                                    {{ old('klijent_id') == $klijent->id ? 'selected' : '' }}>
                                    {{ $klijent->naziv_firme }}
                                </option>
                            @endforeach
                        </select>
                ...
                <div class="row">
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <input type="date" class="form-control @error('rok_isporuke') is-invalid @enderror"
                               ...
                               value="{{ old('rok_isporuke') }}" required>
                        @error('rok_isporuke')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                ...
                <div class="mb-3">
                    ...
                    <select class="form-control @error('status') is-invalid @enderror"
                        ...
                        <option value="nova" {{ old('status', 'nova') == 'nova' ? 'selected' : '' }}>Nova</option>
                        <option value="u_obradi" {{ old('status') == 'u_obradi' ? 'selected' : '' }}>U obradi</option>
                        <option value="zavrsena" {{ old('status') == 'zavrsena' ? 'selected' : '' }}>Završena</option>
                        <option value="otkazana" {{ old('status') == 'otkazana' ? 'selected' : '' }}>Otkazana</option>
                    </select>
                    @error('status')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                ...
                <div class="mb-3">
                    ...
                    <input type="number" step="0.01" class="form-control @error('ukupna_cena') is-invalid @enderror"
                           ...
                           value="{{ old('ukupna_cena', 0) }}" required min="0">
                    @error('ukupna_cena')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="d-flex justify-content-end gap-2">
                    ...
                    <button type="submit" class="btn btn-primary">
                        <i class="fas fa-save"></i> Sačuvaj narudžbinu
                    </button>
                </div>
            </form>
        </div>
...
<script>
    // Automatsko podešavanje roka isporuke (7 dana od današnjeg datuma)
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // Validacija da rok isporuke ne može biti pre datuma narudžbine
        const datumField = document.getElementById('datum_narudzbine');
        datumField.addEventListener('change', function() {
            const datum = new Date(this.value);
            const rok = new Date(rokField.value);

            if (rok < datum) {
                rokField.value = this.value;
            }
        });
    });
</script>
